...
func generateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			gadgetFiles, err := gadget.GenerateGadget(profileName, seccompProfile)
			if err != nil {
				return err
			}

			for filename, data := range gadgetFiles {
				os.WriteFile(opts.outputPath+filename, []byte(data), 0644)
			}
	...
	cmd.Flags().StringVarP(&opts.outputPath, "output", "o", "gadget/", "Path to the output directory to save the generated gadget code")
